...
export default function ContactForm({ questionId, fields }: ContactFormProps) {
  ...
  return (
    <motion.div 
      className="bg-[#1A1A1A] rounded-lg p-3 sm:p-4 md:p-6 shadow-lg w-full"
    ...
    >
      <div className="space-y-3 sm:space-y-4">
          ...
          >
            ...
            {field.type === 'tel' ? (
              ...
              >
                <input
                  ...
                  className="w-full px-3 sm:px-4 py-2.5 sm:py-3 bg-[#2C2C2C] border border-[#FFE44D]/30 rounded-lg text-white text-sm sm:text-base focus:outline-none focus:border-[#FFE44D] transition-all duration-200"
                  onFocus={() => setFocusedField(field.name)}
                  onBlur={() => setFocusedField(null)}
                />
              </motion.div>
            ...
            {getValue(field.name) && (
              <motion.div
                ...
                className="absolute right-2 sm:right-3 top-6 sm:top-7 md:top-9 text-green-500"
              >
        ...
        <motion.div 
          className="text-xs text-gray-400 text-center mt-2 sm:mt-3 pb-1 sm:pb-2"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
